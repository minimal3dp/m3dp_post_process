...
class QualityOptimizer:
    ...
    def __init__(self, segments: List[Segment], config: Optional[QualityConfig] = None):
        """
    ...
    def _identify_loops(self, segments: List[Segment]) -> List[List[Segment]]:
        ...
        for i, seg in enumerate(segments):
            ...
            for j, next_seg in enumerate(segments[i+1:], start=i+1):
                ...
                next_start = get_segment_end_point(segments[j-1]) if j > 0 else get_segment_end_point(next_seg)
