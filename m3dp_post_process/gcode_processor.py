...
class GCodeParser:
    def __init__(self, file_path: Optional[Path] = None, content: Optional[str] = None):
    ...
    def _handle_move(self, parts: List[str], original_text: str, line_num: int):
        ...
        start_point = Point(self.current_pos.x, self.current_pos.y, self.current_pos.z, self.current_pos.e)
...
class Optimizer:
    ...
    def optimize_travel_greedy(self) -> List[Segment]:
        """
        Optimize travel moves using a greedy algorithm.
        1. Group segments into continuous extrusion blocks (islands).
        2. Reorder blocks to minimize travel distance.
        3. Reconstruct segments with new travel moves.
        """
        blocks = []
        current_block = []
        ...
        for seg in self.segments:
            if seg.type == SegmentType.EXTRUSION:
                ...
                if current_block:
                    ...
                    current_block = []
                # We ignore original travel moves as we'll regenerate them
                # But we should keep Z_HOP or other special moves attached to blocks?
                # For simplicity V1: Just reorder extrusion blocks and insert direct travel.
                # TODO: Handle Z-hops and Retractions better.
        ...
        for block in ordered_blocks:
            ...
            if start_point.x != current_pos.x or start_point.y != current_pos.y:
                new_segments.append(Segment(
                    ...
                    speed=3000, # Default travel speed, should be parameterized
                    line_number=-1,
