...
from m3dp_post_process.quality_optimizer import QualityConfig, QualityOptimizer, SeamStrategy
...
def create_segment(x, y, z, e, seg_type=SegmentType.EXTRUSION, text=""):
    ...
    return Segment(
        start=Point(
            x=x - 1 if x > 0 else 0, y=y - 1 if y > 0 else 0, z=z, e=e - 0.1 if e > 0 else 0
        ),
...
def test_quality_config_custom():
    ...
    config = QualityConfig(
        seam_strategy=SeamStrategy.ALIGNED, reduce_shell_crossings=False, min_loop_segments=5
    )
