from m3dp_post_process.gcode_processor import Optimizer, Point, Segment, SegmentType
...
def test_greedy_optimization():
    ...
    assert result.segments[1] == block1_seg  # First extrusion block should be Block 1
    assert result.segments[3] == block2_seg  # Second extrusion block should be Block 2
    assert result.optimization_type == "Travel (Speed)"
...
def test_already_optimal():
    ...
    assert result.segments[1] == block1_seg
    assert result.segments[3] == block2_seg
