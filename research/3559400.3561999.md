...
less travel movement (during which material is not extracted) of the printer head, as separate “islands” of the model are printed
...
extruding material between separated “islands” of the sliced layers.
...
tive layers printed within the same “island” without requiring any
...
printing.
CCS CONCEPTS
• Computing methodologies →Mesh models; • Hardware →
...
on Computational Fabrication (SCF ’22), October 26–28, 2022, Seattle, WA, USA.
...
layers are divided into “islands” (See Figure 2). In each layer, the
...
Travel Movements, i.e., movements for which the printer’s head
...
the printed model’s topmost layer, and therefore collisions might
...
model, each layer is divided into two “islands.” The printer’s
head moves between these “islands” in each printing layer.
A travel movement (marked in blue) connects the “islands”.
...
ties and thermal conductivity play a major role in the printed part’s
...
mercial 3-axis printers’ heads are short and wide. Adaptations to the
...
2014a] the concept of “low-fi fabrication” is introduced as a method
...
example, the “vase mode” In the Cura slicing software [McNeel and
...
is lost toward the nozzle’s tip. When extending the length of the
...
the printer’s screen. The thermal image photos show a decrease of
...
mesh is oriented with the positive ˆ𝑧direction pointing “up”. As is
...
each such closed polygon as a path 𝛾𝑖
𝑗, where i denotes the layer,
...
we first compute the ˆ𝑧iso-lines of the mesh, and then extract the
...
properties:
• All the points on all the paths 𝛾𝑖
𝑗belong to the toolpath.
• The toolpath is printable, i.e., (1) when a path is printed, the
path immediately under it has already been printed, and (2)
the nozzle does not collide with any printed part.
• The toolpath is as short as possible, i.e. travel movement
between paths of different “islands” is minimized.
In classic toolpath generation algorithms, the printability constraints
are respected by guaranteeing that if a path 𝛾𝑖
𝑗appears before the
path 𝛾𝑘
𝑙in the toolpath, then necessarily, 𝑖≤𝑘. This means that all
...
and forth travel between different “islands” – paths on consecutive
...
divide the layers into multi-layers, denoted by 𝑆𝑘, which include a
collection of layers at consecutive heights, whose maximal vertical
distance is given by the nozzle height ℎ𝑚(see figure 5(b)). This
...
the toolpath from the input paths 𝛾𝑖
𝑗. The full problem, where all
...
First, we require a notion of “path z-dependence”, to guarantee that
when a path is printed, the path immediately under it has already
been printed.
Definition 4.1. A path 𝛾𝑖+1
𝑗
z-depends on the path 𝛾𝑖
𝑗if and only
if the XY bounding boxes of 𝛾𝑖
𝑗, 𝛾𝑖+1
𝑗
overlap.
The z-dependence path graph is a directed graph, denoted by
𝐺= (𝑉, 𝐸). Its nodes, V, are all the paths in all the layers 𝛾𝑖
𝑗and
for all i we have that (𝛾𝑖
𝑗,𝛾𝑖+1
𝑘
) ∈𝐸if and only if 𝛾𝑖+1
𝑘
z-depends on
𝛾𝑖
𝑗. Figure 6 shows an example of a model, its paths, multi-layers
...
SCF ’22, October 26–28, 2022, Seattle, WA, USA
4.2.1
Path Starting Points. Let {𝛾1,𝛾2, ...,𝛾𝑁} be the graph traversal
of G induced by the DFS tree T, and denote the starting point of a
path 𝛾by 𝑠(𝛾). For the first path in the traversal, 𝛾1, we choose an
arbitrary starting point. The starting point 𝑠(𝛾𝑘+1) is chosen as the
point on 𝛾𝑘+1 which is closest to 𝑠(𝛾𝑘) (figure 6(d)). The starting
point of the path 𝛾𝑖
𝑗is denoted by 𝑝𝑖
𝑗. This choice of starting points
generates a smooth “seam".
4.3
Travel movement optimization
4.3.1
Optimizing travel movement within a single layer. When the
model branches, a path𝛾𝑖
𝑗will have multiple descendants in the DFS
...
re-order T, such that the successors {𝛾𝑖+1
𝑘1 , ...,𝛾𝑖+1
𝑘𝑙} of each node 𝛾𝑖
𝑗
...
points 𝑝𝑖+1
𝑘1 , ..., 𝑝𝑖+1
𝑘𝑙. We denote the ordered tree by 𝑇𝑂. To propa-
...
on 𝑇𝑂. The result is a DFS order in which the subtrees are ordered
...
of sequential ordering of the print order of each multi-layer 𝑠𝑘. To
obtain this order, we consider the forest𝐹𝑘= 𝑇𝑂[𝑆𝑘], which is the
sub-graph of 𝑇𝑂induced by the paths which belong to 𝑠𝑘(see Fig-
ure 8). To further minimize the travel distance, we reorder the trees
of 𝐹𝑘(i.e., the weakly connected components of 𝐹𝑘), by computing
a Hamiltonian path on the starting points of the lowest path in each
tree. We denote the final order obtained for 𝑠𝑘by 𝜋𝑘= 𝛾1, ...,𝛾𝑚
with 𝛾𝑙∈𝑉.
4.4
...
resulting from (c). (d) The reordered tree𝑇𝑂, computed using
a Hamiltonian path on the branches at one of the middle
layers. (e) The travel movement resulting from (d).
Figure 8: The forest 𝐹𝑘induced by the paths of the multi-
layer 𝑠(𝛾𝑘) (green vertices), with the printing order in each
...
Definition 4.2. A path 𝛾𝑖
𝑗hides the path 𝛾𝑙𝑚if 𝑖≤𝑙and there is
...
dependence graph 𝐻𝑘for the multi-layer 𝑆𝑘. Its vertices are all the
paths in 𝑆𝑘, and an edge (𝛾𝑖
𝑗,𝛾𝑙𝑚) exists if and only if path𝛾𝑙𝑚hides
path 𝛾𝑖
𝑗(figure 10(c)). This graph may contain cycles of paths in the
same layer, as two paths in the same layer might hide each other.
For simplicity, we will first assume that 𝐻𝑘does not have cycles,
and then explain how cycles are handled.
4.4.2
Print order from a directed acyclic graph (DAG). If 𝐻𝑘is a
...
print candidates Γ𝑘which have no incoming edges in 𝐻𝑘. Then,
we repeatedly add a path from Γ𝑘to the printing order, remove it
...
SCF ’22, October 26–28, 2022, Seattle, WA, USA
Kaplan et al.
from the print-dependency graph 𝐻𝑘, and update Γ𝑘accordingly
(see Figure 11, Note that the figure is intended to visualize the
algorithm’s behavior, and thus the starting point is specifically
...
in 𝐻𝑘), it is guaranteed that the generated toolpath is printable. If
there are multiple paths in Γ𝑘, then in terms of printability they
can all be selected to be printed next. We prioritize printing by
the order Π𝑘, computed using Hamiltonian paths in Section 4.3, to
reduce travel movement. Specifically, after adding a path 𝛾to the
printing order, we add its consecutive path in the printing order Π𝑘
if possible (i.e., if it belongs to Γ𝑘). If not, we choose the path from
Γ𝑘which has the earliest printing order in Π𝑘. The final printing
order contains all the paths {𝛾𝑖
𝑗}. We associate with each path a
...
T.
4.4.3
Handling cycles. Cycles in the print-dependency graph 𝐻𝑘
...
of 𝐻𝑘, and denote it by ˆ𝐻𝑘. This graph is a directed acyclic graph
(DAG), where the cycles of 𝐻𝑘become meta-vertices. Thus, we
...
its layers. (c) The print-dependence graph 𝐻1 for the (single)
multi-layer 𝑆1. Note that the right part of the model hides
the left part, which leads to dependencies in 𝐻𝑘between the
left and right parts (dark blue arrows).
Figure 11: Generating the printing order of the paths from
the print-dependence graph 𝐻𝑘. First the paths on the right
side of the model are printed, starting from the bottom (blue)
until we reach a dependency in 𝐻𝑘between the left and right
...
travel movement between “islands” compared to the dimensions of
the model. For example, models with a single “island”, that lead to
...
26mm). The printing speed settings are Cura’s recommended set-
...
retraction distance of 5mm, bed temperature of 60°, and 210° for
...
the Coral in Figure 14. The model’s bounding box diagonal is about
...
of the model’s quality and smoothness can be seen in Figure 16.
6.4
Gallery
...
REFERENCES
Julián I. Aguilar-Duque, Cesar O. Balderrama-Armendáriz, Cesar A. Puente-Montejano,
Arturo S. Ontiveros-Zepeda, Jorge L. García-Alcaraz, and Cesar Balderrama@uacj
...
CEUR-WS (2020), 1–9. https://doi.org/10.1145/nnnnnnn.nnnnnnn
...
Chi K. Tse. 15, 4 (April 2019), 2277–2287. https://doi.org/10.1109/TII.2018.2889740
...
Technology 99, 1 (October 2018), 845–858. https://doi.org/10.1007/s00170-018-2518-
4
...
Technologies. 777–785. https://doi.org/10.1007/978-981-15-2647-3_73
Gierad Laput, Xiang ‘Anthony’ Chen, and Chris Harrison. 2015. 3D Printed Hair:
...
Technology, Association for Computing Machinery, Inc, 167–173.
...
Accuracy of 3 (2018), 258–262. https://doi.org/10.1007/978-3-319-92270-6_36
...
Extruder for Multi-axis 3D Printing. Strojniški vestnik – Journal of Mechanical
Engineering (April 2021 (2021), 167–179. https://doi.org/10.5545/sv-jme.2021.7124
...
François Guimbretière, and Patrick Baudisch. 2014a. WirePrint: 3D printed previews
...
Systems - Proceedings. Association for Computing Machinery, 3827–3834.
...
(2021), 1–19. https://doi.org/10.4236/oalib.1107698
